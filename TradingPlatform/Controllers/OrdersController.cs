...
namespace TradingPlatform.Controllers
{
    ...
    public class OrdersController : BaseController
    {
        ...
        public OrdersController(ApplicationUserManager userManager, ApplicationDbContext _db)
        {
        ...
        public ActionResult Index(bool actual = true)
        {
            ...
            if (actual)
                model = Db.Trades.ActualOrders().ToList().Select(c => tradeDataLayer.CreateTradeViewModel(c, user));
            else
                model = Db.Trades.ActualOrders().ToList().Select(c => tradeDataLayer.CreateTradeViewModel(c, user));
        ...
        public async Task<ActionResult> Create()
        {
            ...
            if (user.UserContragents.Where(c => c.IsApproved).All(f => !f.IsBuyer) || user.UserContragents.Count < 1)
            {
                ModelState.AddModelError("other", LocalText.Inst.Get("error", "Orders.Create.UserNotBuyerException", "Юридична особа має бути активована як покупець", "Юридическое лицо должно быть активировано как покупатель"));
        ...
        public async Task<ActionResult> Create(NewAuctionViewModel model)
        {
           ...
           // model.SelectedProduct = model?.ProductsList?.FirstOrDefault(c => c.Code == TradeTypes.closeFixed.ToString())??new Areas.Admin.Models.CatalogModel() { Code= "closeFixed" };
            try
            {
                ...
                await EmailFactory.Brodcast(new Broadcast() { Body = "Нова заявка", Subject = "На РТР створено нову пропозицію. Подивіться, може у вас є що запропонувати?" });
        ...
        public async Task<ActionResult> Order(UserOrderViewModel model)
        {
            try
            {
                ...
                foreach (var u in offer.Orders.GroupBy(c => c.Buyer).SelectMany(f => f.Key.ContragentUsers.ToList()).ToList())
                {
                ...
                {
                    await hub.UpdateTradeTable(offer.Id, u.UserName);
                    EmailFactory.SendEmailAsync(new NewReplyToOrder(u, order));
                }
        ...
        public ActionResult AcceptOrder(int id)
        {
            try
            {
                ...
                if (order.Volume > order.Trade.GetFreeOfferVolume())
                    throw new ArgumentException(LocalText.Inst.Get("error", "Orders.AcceptOfferDeniedByVolume", "Прийняти завяку неможливо, її об’єм більший за залишок", "Принять заявку невозможно, ее объём больше, чем остаток"));
...
#warning Insert validation logic
                ...
                {
                    await hub.UpdateTradeTable(order.TradeId, u.UserName);
                    //EmailFactory.SendEmailAsync(new OrderAccept(u, order));
                }
        ...
        //    }
        //    Offer offer = await Db.Orders.FindAsync(id);
        //    if (offer == null)
        //    {
        //        return HttpNotFound();
        //    }
